...
router.get("/", checkAuth(Role.owner, Role.admin, Role.manager), ExchangeController.getAllExchanges);
router.post("/", checkAuth(Role.owner, Role.admin), validateRequest(createExchangeZodSchema), ExchangeController.createExchange);
router.delete("/:id", checkAuth(Role.owner), ExchangeController.deleteExchange);
