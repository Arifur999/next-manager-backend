...
// Who is on what is not sensitive - a member benefits from seeing the team
...
router.post("/", checkAuth(Role.owner, Role.admin, Role.manager), validateRequest(assignMemberZodSchema), ProjectMemberController.assignMember);
router.delete("/:id", checkAuth(Role.owner, Role.admin, Role.manager), ProjectMemberController.removeMember);
